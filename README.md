...
고르시면 됩니다. 스치고 지나가는 것과 구분하려고 0.25초 머물러야 열리고, 이렇게 열린
창은 마우스가 충분히 멀어지면 스스로 닫힙니다. 항목을 끌고 있는 동안에는 닫히지
않으므로 다른 앱으로 드래그하는 데 지장이 없습니다.
...
| `EdgeHoverMonitor.swift` | 마우스가 화면 끝에 머무는지 감지 |
